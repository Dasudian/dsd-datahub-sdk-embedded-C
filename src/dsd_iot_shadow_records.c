...
static void AckStatusCallback(DSD_IoT_Client *pClient, char *topicName,
							  uint16_t topicNameLen, IoT_Publish_Message_Params *params, void *pData);

static void shadow_delta_callback(DSD_IoT_Client *pClient, char *topicName,
								  uint16_t topicNameLen, IoT_Publish_Message_Params *params, void *pData);
...
IoT_Error_t registerJsonTokenOnDelta(jsonStruct_t *pStruct) {
	...
	if(!deltaTopicSubscribedFlag) {
		snprintf(shadowDeltaTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, "$dsd/things/%s/shadow/update/delta", myThingName);
		rc = dsd_iot_mqtt_subscribe(pMqttClient, shadowDeltaTopic, (uint16_t) strlen(shadowDeltaTopic), QOS0,
...
static void topicNameFromThingAndAction(char *pTopic, const char *pThingName, ShadowActions_t action,
	...
	if(SHADOW_ACTION == ackType) {
		snprintf(pTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, "$dsd/things/%s/shadow/%s", pThingName, actionBuf);
	} else {
		snprintf(pTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, "$dsd/things/%s/shadow/%s/%s", pThingName, actionBuf,
				 ackTypeBuf);
	}
}
...
static void AckStatusCallback(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
...
static void unsubscribeFromAcceptedAndRejected(uint8_t index) {
	...
	if((indexSubList >= 0)) {
		if(!SubscriptionList[indexSubList].isSticky && (SubscriptionList[indexSubList].count == 1)) {
			ret_val = dsd_iot_mqtt_unsubscribe(pMqttClient, TemporaryTopicNameAccepted,
	...
	if((indexSubList >= 0)) {
		if(!SubscriptionList[indexSubList].isSticky && (SubscriptionList[indexSubList].count == 1)) {
			ret_val = dsd_iot_mqtt_unsubscribe(pMqttClient, TemporaryTopicNameRejected,
...
void initializeRecords(DSD_IoT_Client *pClient) {
	uint8_t i;
	for(i = 0; i < MAX_ACKS_TO_COMEIN_AT_ANY_GIVEN_TIME; i++) {
		AckWaitList[i].isFree = true;
	}
...
IoT_Error_t subscribeToShadowActionAcks(const char *pThingName, ShadowActions_t action, bool isSticky) {
	...
	if(indexAcceptedSubList >= 0 && indexRejectedSubList >= 0) {
		...
		ret_val = dsd_iot_mqtt_subscribe(pMqttClient, SubscriptionList[indexAcceptedSubList].Topic,
		...
		if(ret_val == SUCCESS) {
			...
			topicNameFromThingAndAction(SubscriptionList[indexRejectedSubList].Topic, pThingName, action,
										SHADOW_REJECTED);
			ret_val = dsd_iot_mqtt_subscribe(pMqttClient, SubscriptionList[indexRejectedSubList].Topic,
	...
	if(clearBothEntriesFromList) {
		if(indexAcceptedSubList >= 0) {
			...
			if(SubscriptionList[indexAcceptedSubList].count == 1) {
			    dsd_iot_mqtt_unsubscribe(pMqttClient, SubscriptionList[indexAcceptedSubList].Topic,
...
IoT_Error_t publishToShadowAction(const char *pThingName, ShadowActions_t action, const char *pJsonDocumentToBeSent) {
	...
	ret_val = dsd_iot_mqtt_publish(pMqttClient, TemporaryTopicName, (uint16_t) strlen(TemporaryTopicName), &msgParams);
