...
/**
 * @file dsd_iot_mqtt_client_yield.c
...
/**
  * This is for the case when the dsd_iot_mqtt_internal_send_packet Fails.
  */
static void _dsd_iot_mqtt_force_client_disconnect(DSD_IoT_Client *pClient) {
...
static IoT_Error_t _dsd_iot_mqtt_handle_disconnect(DSD_IoT_Client *pClient) {
	IoT_Error_t rc;

	FUNC_ENTRY;

	rc = dsd_iot_mqtt_disconnect(pClient);
	if(rc != SUCCESS) {
		// If the dsd_iot_mqtt_internal_send_packet prevents us from sending a disconnect packet then we have to clean the stack
		_dsd_iot_mqtt_force_client_disconnect(pClient);
	}
...
static IoT_Error_t _dsd_iot_mqtt_handle_reconnect(DSD_IoT_Client *pClient) {
	IoT_Error_t rc;
	...
	if(DSD_IOT_MQTT_MAX_RECONNECT_WAIT_INTERVAL < pClient->clientData.currentReconnectWaitInterval) {
...
static IoT_Error_t _dsd_iot_mqtt_keep_alive(DSD_IoT_Client *pClient) {
	...
	if(pClient->clientStatus.isPingOutstanding) {
		rc = _dsd_iot_mqtt_handle_disconnect(pClient);
		FUNC_EXIT_RC(rc);
	}
	...
	rc = dsd_iot_mqtt_internal_send_packet(pClient, serialized_len, &timer);
	if(SUCCESS != rc) {
...
static IoT_Error_t _dsd_iot_mqtt_internal_yield(DSD_IoT_Client *pClient, uint32_t timeout_ms) {
	...
	do {
		...
		if(CLIENT_STATE_PENDING_RECONNECT == clientState) {
			...
			yieldRc = _dsd_iot_mqtt_handle_reconnect(pClient);
			/* Network reconnect attempted, check if yield timer expired before
			 * doing anything else */
			continue;
		}

		yieldRc = dsd_iot_mqtt_internal_cycle_read(pClient, &timer, &packet_type);
		if(SUCCESS == yieldRc) {
			yieldRc = _dsd_iot_mqtt_keep_alive(pClient);
		} else {
			...
			if(NETWORK_SSL_READ_ERROR == yieldRc || NETWORK_SSL_WRITE_ERROR == yieldRc || NETWORK_SSL_WRITE_TIMEOUT_ERROR == yieldRc) {
				yieldRc = _dsd_iot_mqtt_handle_disconnect(pClient);
			}
		}
		...
		if(NETWORK_DISCONNECTED_ERROR == yieldRc) {
			...
			if(1 == pClient->clientStatus.isAutoReconnectEnabled) {
				yieldRc = dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_DISCONNECTED_ERROR,
														CLIENT_STATE_PENDING_RECONNECT);
				if(SUCCESS != yieldRc) {
					FUNC_EXIT_RC(yieldRc);
				}

				pClient->clientData.currentReconnectWaitInterval = DSD_IOT_MQTT_MIN_RECONNECT_WAIT_INTERVAL;
				countdown_ms(&(pClient->reconnectDelayTimer), pClient->clientData.currentReconnectWaitInterval);
 ...
 *         iot_is_mqtt_connected can be called to confirm.
 */
IoT_Error_t dsd_iot_mqtt_yield(DSD_IoT_Client *pClient, uint32_t timeout_ms) {
	...
	yieldRc = _dsd_iot_mqtt_internal_yield(pClient, timeout_ms);

	if(NETWORK_DISCONNECTED_ERROR != yieldRc && NETWORK_ATTEMPTING_RECONNECT != yieldRc) {
		rc = dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_YIELD_IN_PROGRESS,
