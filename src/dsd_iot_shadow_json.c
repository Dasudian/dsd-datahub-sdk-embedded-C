...
#define DSD_IOT_SHADOW_CLIENT_TOKEN_KEY "{\"clientToken\":\""
...
static IoT_Error_t emptyJsonWithClientToken(char *pBuffer, size_t bufferSize) {
	...
	if(pBuffer != NULL)
    {
        dataLenInBuffer = (size_t)snprintf(pBuffer, bufferSize, DSD_IOT_SHADOW_CLIENT_TOKEN_KEY);
    }else
	{
...
IoT_Error_t dsd_iot_shadow_internal_get_request_json(char *pBuffer, size_t bufferSize) {
	return emptyJsonWithClientToken( pBuffer, bufferSize);
}

IoT_Error_t dsd_iot_shadow_internal_delete_request_json(char *pBuffer, size_t bufferSize ) {
	return emptyJsonWithClientToken( pBuffer, bufferSize);
}
...
IoT_Error_t dsd_iot_shadow_init_json_document(char *pJsonDocument, size_t maxSizeOfJsonDocument) {
...
IoT_Error_t dsd_iot_shadow_add_desired(char *pJsonDocument, size_t maxSizeOfJsonDocument, uint8_t count, ...) {
	IoT_Error_t ret_val = SUCCESS;
	size_t tempSize = 0;
	int8_t i;
...
IoT_Error_t dsd_iot_shadow_add_reported(char *pJsonDocument, size_t maxSizeOfJsonDocument, uint8_t count, ...) {
	IoT_Error_t ret_val = SUCCESS;
...
IoT_Error_t dsd_iot_fill_with_client_token(char *pBufferToBeUpdatedWithClientToken, size_t maxSizeOfJsonDocument) {
...
IoT_Error_t dsd_iot_finalize_json_document(char *pJsonDocument, size_t maxSizeOfJsonDocument) {
	size_t remSizeOfJsonBuffer = maxSizeOfJsonDocument;
	int32_t snPrintfReturn = 0;
	size_t tempSize = 0;
	IoT_Error_t ret_val = SUCCESS;
