...
/**
 * @file dsd_iot_mqtt_client_common_internal.c
...
#include "../include/dsd_iot_mqtt_client.h"
#include "../include/dsd_iot_mqtt_client_common_internal.h"
 ...
 */
size_t dsd_iot_mqtt_internal_write_len_to_buffer(unsigned char *buf, uint32_t length) {
	size_t outLen = 0;
 ...
 */
IoT_Error_t dsd_iot_mqtt_internal_decode_remaining_length_from_buffer(unsigned char *buf, uint32_t *decodedLen,
...
uint32_t dsd_iot_mqtt_internal_get_final_packet_length_from_remaining_length(uint32_t rem_len) {
 ...
 */
uint16_t dsd_iot_mqtt_internal_read_uint16_t(unsigned char **pptr) {
 ...
 */
void dsd_iot_mqtt_internal_write_uint_16(unsigned char **pptr, uint16_t anInt) {
 ...
 */
unsigned char dsd_iot_mqtt_internal_read_char(unsigned char **pptr) {
 ...
 */
void dsd_iot_mqtt_internal_write_char(unsigned char **pptr, unsigned char c) {
	**pptr = c;
	(*pptr)++;
}

void dsd_iot_mqtt_internal_write_utf8_string(unsigned char **pptr, const char *string, uint16_t stringLen) {
	/* Nothing that calls this function will have a stringLen with a size larger than 2 bytes (MQTT 3.1.1 - 1.5.3) */
	dsd_iot_mqtt_internal_write_uint_16(pptr, stringLen);
	if(stringLen > 0) {
 ...
 */
IoT_Error_t dsd_iot_mqtt_internal_init_header(MQTTHeader *pHeader, MessageTypes message_type,
...
IoT_Error_t dsd_iot_mqtt_internal_send_packet(DSD_IoT_Client *pClient, size_t length, Timer *pTimer) {
	...
	rc = dsd_iot_mqtt_client_lock_mutex(pClient, &(pClient->clientData.tls_write_mutex));
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
#endif
...
#ifdef _ENABLE_THREAD_SUPPORT_
	rc = dsd_iot_mqtt_client_unlock_mutex(pClient, &(pClient->clientData.tls_write_mutex));
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
#endif
...
static IoT_Error_t _dsd_iot_mqtt_internal_readWrapper( DSD_IoT_Client *pClient, size_t offset, size_t size, Timer *pTimer, size_t * read_len ) {
    IoT_Error_t rc;
    int byteToRead;
    size_t byteRead = 0;
...
static IoT_Error_t _dsd_iot_mqtt_internal_decode_packet_remaining_len(DSD_IoT_Client *pClient, size_t * offset,
	...
	*rem_len = 0;
	...
	do {
        ...
        rc = _dsd_iot_mqtt_internal_readWrapper( pClient, len, 1, pTimer, &read_len );
...
static IoT_Error_t _dsd_iot_mqtt_internal_read_packet(DSD_IoT_Client *pClient, Timer *pTimer, uint8_t *pPacketType) {
    ...
    rc = _dsd_iot_mqtt_internal_readWrapper( pClient, offset, 1, pTimer, &read_len );
	...
	rc = _dsd_iot_mqtt_internal_decode_packet_remaining_len(pClient, &offset, &rem_len, pTimer);
	if(SUCCESS != rc) {
		return rc;
	} 
	...
	if((rem_len + offset) >= pClient->clientData.readBufSize) {
        ...
        {
            dsd_iot_mqtt_internal_flushBuffers( pClient );
	...
	if(rem_len > 0) {
        rc = _dsd_iot_mqtt_internal_readWrapper( pClient, offset, rem_len, pTimer, &read_len );
...
static bool _dsd_iot_mqtt_internal_is_topic_matched(char *pTopicFilter, char *pTopicName, uint16_t topicNameLen) {
...
static IoT_Error_t _dsd_iot_mqtt_internal_deliver_message(DSD_IoT_Client *pClient, char *pTopicName,
	...
	clientState = dsd_iot_mqtt_get_client_state(pClient);
	dsd_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTED_WAIT_FOR_CB_RETURN);

	/* Find the right message handler - indexed by topic */
	for(itr = 0; itr < DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS; ++itr) {
		if(NULL != pClient->clientData.messageHandlers[itr].topicName) {
			if(((topicNameLen == pClient->clientData.messageHandlers[itr].topicNameLen)
			   ...
			   || _dsd_iot_mqtt_internal_is_topic_matched((char *) pClient->clientData.messageHandlers[itr].topicName,
	...
	rc = dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_WAIT_FOR_CB_RETURN, clientState);

	FUNC_EXIT_RC(rc);
}

static IoT_Error_t _dsd_iot_mqtt_internal_handle_publish(DSD_IoT_Client *pClient, Timer *pTimer) {
	char *topicName;
	uint16_t topicNameLen;
	uint32_t len;
	IoT_Error_t rc;
	...
	rc = dsd_iot_mqtt_internal_deserialize_publish(&msg.isDup, &msg.qos, &msg.isRetained,
	...
	rc = _dsd_iot_mqtt_internal_deliver_message(pClient, topicName, topicNameLen, &msg);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	...
	rc = dsd_iot_mqtt_internal_serialize_ack(pClient->clientData.writeBuf, pClient->clientData.writeBufSize,
											 PUBACK, 0, msg.id, &len);
	...
	rc = dsd_iot_mqtt_internal_send_packet(pClient, len, pTimer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
...
IoT_Error_t dsd_iot_mqtt_internal_cycle_read(DSD_IoT_Client *pClient, Timer *pTimer, uint8_t *pPacketType) {
	IoT_Error_t rc;
...
#ifdef _ENABLE_THREAD_SUPPORT_
	threadRc = dsd_iot_mqtt_client_lock_mutex(pClient, &(pClient->clientData.tls_read_mutex));
	if(SUCCESS != threadRc) {
	...
	rc = _dsd_iot_mqtt_internal_read_packet(pClient, pTimer, pPacketType);

#ifdef _ENABLE_THREAD_SUPPORT_
	threadRc = dsd_iot_mqtt_client_unlock_mutex(pClient, &(pClient->clientData.tls_read_mutex));
	...
	switch(*pPacketType) {
		...
		case UNSUBACK:
			...
			break;
		case PUBLISH: {
			rc = _dsd_iot_mqtt_internal_handle_publish(pClient, pTimer);
			break;
		}
		case PUBREC:
		case PUBCOMP:
...
IoT_Error_t dsd_iot_mqtt_internal_flushBuffers( DSD_IoT_Client *pClient ) {
    pClient->clientData.readBufIndex = 0;
    return SUCCESS;
}

/* only used in single-threaded mode where one command at a time is in process */
IoT_Error_t dsd_iot_mqtt_internal_wait_for_read(DSD_IoT_Client *pClient, uint8_t packetType, Timer *pTimer) {
	IoT_Error_t rc;
	...
	do {
		...
		rc = dsd_iot_mqtt_internal_cycle_read(pClient, pTimer, &read_packet_type);
...
/**
  ...
  */
IoT_Error_t dsd_iot_mqtt_internal_serialize_zero(unsigned char *pTxBuf, size_t txBufLen, MessageTypes packetType,
	...
	rc = dsd_iot_mqtt_internal_init_header(&header, packetType, QOS0, 0, 0);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* write header */
	dsd_iot_mqtt_internal_write_char(&ptr, header.byte);

	/* write remaining length */
	ptr += dsd_iot_mqtt_internal_write_len_to_buffer(ptr, 0);
