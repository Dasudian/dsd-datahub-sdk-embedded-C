...
/**
 * @file dsd_iot_mqtt_client_subscribe.c
...
/**
  ...
  */
static IoT_Error_t _dsd_iot_mqtt_serialize_subscribe(unsigned char *pTxBuf, size_t txBufLen,
	...
	if(dsd_iot_mqtt_internal_get_final_packet_length_from_remaining_length(rem_len) > txBufLen) {
		FUNC_EXIT_RC(MQTT_TX_BUFFER_TOO_SHORT_ERROR);
	}

	rc = dsd_iot_mqtt_internal_init_header(&header, SUBSCRIBE, QOS1, dup, 0);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	/* write header */
	...
	ptr += dsd_iot_mqtt_internal_write_len_to_buffer(ptr, rem_len);

	dsd_iot_mqtt_internal_write_uint_16(&ptr, packetId);

	for(itr = 0; itr < topicCount; ++itr) {
		dsd_iot_mqtt_internal_write_utf8_string(&ptr, pTopicNameList[itr], pTopicNameLenList[itr]);
		dsd_iot_mqtt_internal_write_char(&ptr, (unsigned char) pRequestedQoSs[itr]);
	}
...
/**
  ...
  * @return An IoT Error Type defining successful/failed operation
  */
static IoT_Error_t _dsd_iot_mqtt_deserialize_suback(uint16_t *pPacketId, uint32_t maxExpectedQoSCount,
	...
	decodeRc = dsd_iot_mqtt_internal_decode_remaining_length_from_buffer(curData, &decodedLen, &readBytesLen);
	if(SUCCESS != decodeRc) {
	...
	*pPacketId = dsd_iot_mqtt_internal_read_uint16_t(&curData);
	...
	*pGrantedQoSCount = 0;
	while(curData < endData) {
		...
		pGrantedQoSs[(*pGrantedQoSCount)++] = (QoS) dsd_iot_mqtt_internal_read_char(&curData);
	}
...
static uint32_t _dsd_iot_mqtt_get_free_message_handler_index(DSD_IoT_Client *pClient) {
	uint32_t itr;
 ...
 */
static IoT_Error_t _dsd_iot_mqtt_internal_subscribe(DSD_IoT_Client *pClient, const char *pTopicName,
	...
	serializedLen = 0;
	count = 0;
	txPacketId = dsd_iot_mqtt_get_next_packet_id(pClient);
	rxPacketId = 0;

	rc = _dsd_iot_mqtt_serialize_subscribe(pClient->clientData.writeBuf, pClient->clientData.writeBufSize, 0,
										   txPacketId, 1, &pTopicName, &topicNameLen, &qos, &serializedLen);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	indexOfFreeMessageHandler = _dsd_iot_mqtt_get_free_message_handler_index(pClient);
	if(DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS <= indexOfFreeMessageHandler) {
		FUNC_EXIT_RC(MQTT_MAX_SUBSCRIPTIONS_REACHED_ERROR);
	}

	/* send the subscribe packet */
	rc = dsd_iot_mqtt_internal_send_packet(pClient, serializedLen, &timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* wait for suback */
	rc = dsd_iot_mqtt_internal_wait_for_read(pClient, SUBACK, &timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* Granted QoS can be 0, 1 or 2 */
	rc = _dsd_iot_mqtt_deserialize_suback(&rxPacketId, 1, &count, grantedQoS, pClient->clientData.readBuf,
 ...
 * @return An IoT Error Type defining successful/failed subscription
 */
IoT_Error_t dsd_iot_mqtt_subscribe(DSD_IoT_Client *pClient, const char *pTopicName, uint16_t topicNameLen,
	...
	rc = dsd_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTED_SUBSCRIBE_IN_PROGRESS);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	subRc = _dsd_iot_mqtt_internal_subscribe(pClient, pTopicName, topicNameLen, qos,
											 pApplicationHandler, pApplicationHandlerData);

	rc = dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_SUBSCRIBE_IN_PROGRESS, clientState);
 ...
 * @return An IoT Error Type defining successful/failed subscription
 */
static IoT_Error_t _dsd_iot_mqtt_internal_resubscribe(DSD_IoT_Client *pClient) {
	uint16_t packetId;
	...
	len = 0;
	count = 0;
	existingSubCount = _dsd_iot_mqtt_get_free_message_handler_index(pClient);
	...
	for(itr = 0; itr < existingSubCount; itr++) {
		...
		rc = _dsd_iot_mqtt_serialize_subscribe(pClient->clientData.writeBuf, pClient->clientData.writeBufSize, 0,
											   dsd_iot_mqtt_get_next_packet_id(pClient), 1,
		...
		/* send the subscribe packet */
		rc = dsd_iot_mqtt_internal_send_packet(pClient, len, &timer);
		if(SUCCESS != rc) {
			FUNC_EXIT_RC(rc);
		}
		...
		/* Granted QoS can be 0, 1 or 2 */
		rc = _dsd_iot_mqtt_deserialize_suback(&packetId, 1, &count, grantedQoS, pClient->clientData.readBuf,
											  pClient->clientData.readBufSize);
		if(SUCCESS != rc) {
			FUNC_EXIT_RC(rc);
		}
 ...
 * @return An IoT Error Type defining successful/failed subscription
 */
IoT_Error_t dsd_iot_mqtt_resubscribe(DSD_IoT_Client *pClient) {
	...
	if(false == dsd_iot_mqtt_is_client_connected(pClient)) {
		FUNC_EXIT_RC(NETWORK_DISCONNECTED_ERROR);
	}
	...
	rc = dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_IDLE,
									   CLIENT_STATE_CONNECTED_RESUBSCRIBE_IN_PROGRESS);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	resubRc = _dsd_iot_mqtt_internal_resubscribe(pClient);

	rc = dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_RESUBSCRIBE_IN_PROGRESS,
