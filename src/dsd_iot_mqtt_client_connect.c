...
/**
 * @file dsd_iot_mqtt_client_connect.c
...
/**
  ...
  */
static uint32_t _dsd_iot_get_connect_packet_length(IoT_Client_Connect_Params *pConnectParams) {
	uint32_t len;
...
/**
  ...
  * @return IoT_Error_t indicating function execution status
  */
static IoT_Error_t _dsd_iot_mqtt_serialize_connect(unsigned char *pTxBuf, size_t txBufLen,
	...
	ptr = pTxBuf;
	len = _dsd_iot_get_connect_packet_length(pConnectParams);
	if(dsd_iot_mqtt_internal_get_final_packet_length_from_remaining_length(len) > txBufLen) {
		FUNC_EXIT_RC(MQTT_TX_BUFFER_TOO_SHORT_ERROR);
	}

	rc = dsd_iot_mqtt_internal_init_header(&header, CONNECT, QOS0, 0, 0);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	dsd_iot_mqtt_internal_write_char(&ptr, header.byte); /* write header */

	ptr += dsd_iot_mqtt_internal_write_len_to_buffer(ptr, len); /* write remaining length */

	// Enable if adding support for more versions
	//if(MQTT_3_1_1 == pConnectParams->MQTTVersion) {
	dsd_iot_mqtt_internal_write_utf8_string(&ptr, "MQTT", 4);
	dsd_iot_mqtt_internal_write_char(&ptr, (unsigned char) pConnectParams->MQTTVersion);
	//}
	...
	dsd_iot_mqtt_internal_write_char(&ptr, flags.all);
	dsd_iot_mqtt_internal_write_uint_16(&ptr, pConnectParams->keepAliveIntervalInSec);
	...
	if(NULL == pConnectParams->pClientID) {
		dsd_iot_mqtt_internal_write_uint_16(&ptr, 0);
	} else {
		dsd_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->pClientID, pConnectParams->clientIDLen);
	}

	if(pConnectParams->isWillMsgPresent) {
		dsd_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->will.pTopicName,
												pConnectParams->will.topicNameLen);
		dsd_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->will.pMessage, pConnectParams->will.msgLen);
	}

	if(pConnectParams->pUsername) {
		dsd_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->pUsername, pConnectParams->usernameLen);
	}

	if(pConnectParams->pPassword) {
		dsd_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->pPassword, pConnectParams->passwordLen);
	}
...
/**
  ...
  * @return IoT_Error_t indicating function execution status
  */
static IoT_Error_t _dsd_iot_mqtt_deserialize_connack(unsigned char *pSessionPresent, IoT_Error_t *pConnackRc,
	...
	header.byte = dsd_iot_mqtt_internal_read_char(&curdata);
	if(CONNACK != MQTT_HEADER_FIELD_TYPE(header.byte)) {
		FUNC_EXIT_RC(FAILURE);
	}

	/* read remaining length */
	rc = dsd_iot_mqtt_internal_decode_remaining_length_from_buffer(curdata, &decodedLen, &readBytesLen);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	...
	flags.all = dsd_iot_mqtt_internal_read_char(&curdata);
	*pSessionPresent = flags.bits.sessionpresent;
	connack_rc_char = dsd_iot_mqtt_internal_read_char(&curdata);
 ...
 */
static bool _dsd_iot_mqtt_is_client_state_valid_for_connect(ClientState clientState) {
	bool isValid = false;
 ...
 *
 * Called to establish an MQTT connection with the DSD IoT Service
 ...
 */
static IoT_Error_t _dsd_iot_mqtt_internal_connect(DSD_IoT_Client *pClient, IoT_Client_Connect_Params *pConnectParams) {
	Timer connect_timer;
	...
	if(NULL != pConnectParams) {
		...
		rc = dsd_iot_mqtt_set_connect_params(pClient, pConnectParams);
		if(SUCCESS != rc) {
	...
	rc = _dsd_iot_mqtt_serialize_connect(pClient->clientData.writeBuf, pClient->clientData.writeBufSize,
	...
	rc = dsd_iot_mqtt_internal_send_packet(pClient, len, &connect_timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* this will be a blocking call, wait for the CONNACK */
	rc = dsd_iot_mqtt_internal_wait_for_read(pClient, CONNACK, &connect_timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* Received CONNACK, check the return code */
	rc = _dsd_iot_mqtt_deserialize_connack((unsigned char *) &sessionPresent, &connack_rc, pClient->clientData.readBuf,
 ...
 * @return An IoT Error Type defining successful/failed connection
 */
IoT_Error_t dsd_iot_mqtt_connect(DSD_IoT_Client *pClient, IoT_Client_Connect_Params *pConnectParams) {
	...
	if(false == _dsd_iot_mqtt_is_client_state_valid_for_connect(clientState)) {
	...
	dsd_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTING);

	rc = _dsd_iot_mqtt_internal_connect(pClient, pConnectParams);
	...
	if(SUCCESS != rc) {
		...
		dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTING, CLIENT_STATE_DISCONNECTED_ERROR);
	} else {
		dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTING, CLIENT_STATE_CONNECTED_IDLE);
	}
 ...
 */
static IoT_Error_t _dsd_iot_mqtt_internal_disconnect(DSD_IoT_Client *pClient) {
	...
	rc = dsd_iot_mqtt_internal_serialize_zero(pClient->clientData.writeBuf, pClient->clientData.writeBufSize,
											  DISCONNECT,
											  &serialized_len);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	...
	if(serialized_len > 0) {
		(void)dsd_iot_mqtt_internal_send_packet(pClient, serialized_len, &timer);
	}
 ...
 * @return An IoT Error Type defining successful/failed send of the disconnect control packet.
 */
IoT_Error_t dsd_iot_mqtt_disconnect(DSD_IoT_Client *pClient) {
	ClientState clientState;
	IoT_Error_t rc;
	...
	rc = dsd_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_DISCONNECTING);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	rc = _dsd_iot_mqtt_internal_disconnect(pClient);
 ...
 * @return An IoT Error Type defining successful/failed connection
 */
IoT_Error_t dsd_iot_mqtt_attempt_reconnect(DSD_IoT_Client *pClient) {
	IoT_Error_t rc;
	...
	rc = dsd_iot_mqtt_connect(pClient, NULL);

	/* If still disconnected handle disconnect */
	if(CLIENT_STATE_CONNECTED_IDLE != dsd_iot_mqtt_get_client_state(pClient)) {
		dsd_iot_mqtt_set_client_state(pClient, CLIENT_STATE_DISCONNECTED_ERROR, CLIENT_STATE_PENDING_RECONNECT);
		FUNC_EXIT_RC(NETWORK_ATTEMPTING_RECONNECT);
	}

	rc = dsd_iot_mqtt_resubscribe(pClient);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
