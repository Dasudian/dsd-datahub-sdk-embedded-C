...
IoT_Client_Connect_Params dsd_init_conn_params(char *username, char *password) {
	IoT_Client_Connect_Params a = { {'M', 'Q', 'T', 'C'}, MQTT_3_1_1, NULL, 0, 60, true, false, \
        IoT_MQTT_Will_Options_Initializer, username, (uint16_t) strlen(username), password,(uint16_t) strlen(password) };
	return a;
}

ClientState dsd_iot_mqtt_get_client_state(DSD_IoT_Client *pClient) {
	FUNC_ENTRY;
	if(NULL == pClient) {
...
#ifdef _ENABLE_THREAD_SUPPORT_
IoT_Error_t dsd_iot_mqtt_client_lock_mutex(DSD_IoT_Client *pClient, IoT_Mutex_t *pMutex) {
	FUNC_ENTRY;
	...
	if(false == pClient->clientData.isBlockOnThreadLockEnabled) {
		threadRc = dsd_iot_thread_mutex_trylock(pMutex);
	} else {
		threadRc = dsd_iot_thread_mutex_lock(pMutex);
...
IoT_Error_t dsd_iot_mqtt_client_unlock_mutex(DSD_IoT_Client *pClient, IoT_Mutex_t *pMutex) {
	if(NULL == pClient || NULL == pMutex) {
		return NULL_VALUE_ERROR;
	}
	IOT_UNUSED(pClient);
	return dsd_iot_thread_mutex_unlock(pMutex);
}
#endif

IoT_Error_t dsd_iot_mqtt_set_client_state(DSD_IoT_Client *pClient, ClientState expectedCurrentState,
...
#ifdef _ENABLE_THREAD_SUPPORT_
	rc = dsd_iot_mqtt_client_lock_mutex(pClient, &(pClient->clientData.state_change_mutex));
	if(SUCCESS != rc) {
		return rc;
	}
#endif
	if(expectedCurrentState == dsd_iot_mqtt_get_client_state(pClient)) {
...
#ifdef _ENABLE_THREAD_SUPPORT_
	threadRc = dsd_iot_mqtt_client_unlock_mutex(pClient, &(pClient->clientData.state_change_mutex));
...
IoT_Error_t dsd_iot_mqtt_set_connect_params(DSD_IoT_Client *pClient, IoT_Client_Connect_Params *pNewConnectParams) {
	FUNC_ENTRY;
...
IoT_Error_t dsd_iot_mqtt_free(DSD_IoT_Client *pClient)
{
    ...
    }else
	{
		...
		if (rc == SUCCESS)
		{
			rc = dsd_iot_thread_mutex_destroy(&(pClient->clientData.state_change_mutex));
		}

		if (rc == SUCCESS)
		{
			rc = dsd_iot_thread_mutex_destroy(&(pClient->clientData.tls_read_mutex));
		}else{
			(void)dsd_iot_thread_mutex_destroy(&(pClient->clientData.tls_read_mutex));
		}

		if (rc == SUCCESS)
		{
			rc = dsd_iot_thread_mutex_destroy(&(pClient->clientData.tls_write_mutex));
		}else{
			(void)dsd_iot_thread_mutex_destroy(&(pClient->clientData.tls_write_mutex));
		}
	#endif
	}
...
IoT_Error_t dsd_iot_mqtt_init(DSD_IoT_Client *pClient, IoT_Client_Init_Params *pInitParams, IoT_Client_Connect_Params *pConnParams) {
	uint32_t i;
	IoT_Error_t rc;
	...
	for(i = 0; i < DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS; ++i) {
	...
	pClient->clientData.writeBufSize = DSD_IOT_MQTT_TX_BUF_LEN;
	pClient->clientData.readBufSize = DSD_IOT_MQTT_RX_BUF_LEN;
	...
	rc = dsd_iot_mqtt_set_connect_params(pClient, pConnParams);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	...
	rc = dsd_iot_thread_mutex_init(&(pClient->clientData.state_change_mutex));
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	rc = dsd_iot_thread_mutex_init(&(pClient->clientData.tls_read_mutex));
	if(SUCCESS != rc) {
		(void)dsd_iot_thread_mutex_destroy(&(pClient->clientData.state_change_mutex));
		FUNC_EXIT_RC(rc);
	}
	rc = dsd_iot_thread_mutex_init(&(pClient->clientData.tls_write_mutex));
	if(SUCCESS != rc) {
		(void)dsd_iot_thread_mutex_destroy(&(pClient->clientData.tls_read_mutex));
		(void)dsd_iot_thread_mutex_destroy(&(pClient->clientData.state_change_mutex));
		FUNC_EXIT_RC(rc);
...
uint16_t dsd_iot_mqtt_get_next_packet_id(DSD_IoT_Client *pClient) {
	return pClient->clientData.nextPacketId = (uint16_t) ((MAX_PACKET_ID == pClient->clientData.nextPacketId) ? 1 : (
			pClient->clientData.nextPacketId + 1));
}

bool dsd_iot_mqtt_is_client_connected(DSD_IoT_Client *pClient) {
	bool isConnected;
...
bool dsd_iot_is_autoreconnect_enabled(DSD_IoT_Client *pClient) {
	FUNC_ENTRY;
	if(NULL == pClient) {
		IOT_WARN(" Client is null! ");
		FUNC_EXIT_RC(false);
...
IoT_Error_t dsd_iot_mqtt_autoreconnect_set_status(DSD_IoT_Client *pClient, bool newStatus) {
	FUNC_ENTRY;
	if(NULL == pClient) {
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
...
IoT_Error_t dsd_iot_mqtt_set_disconnect_handler(DSD_IoT_Client *pClient, iot_disconnect_handler pDisconnectHandler,
...
uint32_t dsd_iot_mqtt_get_network_disconnected_count(DSD_IoT_Client *pClient) {
	return pClient->clientData.counterNetworkDisconnected;
}

void dsd_iot_mqtt_reset_network_disconnected_count(DSD_IoT_Client *pClient) {
	pClient->clientData.counterNetworkDisconnected = 0;
}
