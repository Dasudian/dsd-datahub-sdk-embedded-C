 ...
 * Explore the example.  It connects to DSD IoT platform using MQTT and demonstrates few actions that can be performed by the SDK
 ...
 * Ensure you have [created a thing](https://docs.dsd.dasudian.com/iot/latest/developerguide/create-thing.html) through your DSD IoT Console with name matching the definition DSD_IOT_MY_THING_NAME in the `dsd_iot_config.h` file
 ...
 * All samples are written in C unless otherwise mentioned. The following sample applications are included:
	...
	* `shadow_sample_console_echo` - a sample to work with the DSD IoT Console interactive guide
 ...
 * The sample first creates an instance of the DSD_IoT_Client
 * The next step is to initialize the client. The dsd_iot_mqtt_init API is called for this purpose. The API takes the client instance and an IoT_Client_Init_Params variable to set the initial values for the client. The Init params include values like host URL, port, certificates, disconnect handler etc.
 * If the call to the init API was successful, we can proceed to call connect. The API is called dsd_iot_mqtt_connect. It takes the client instance and IoT_Client_Connect_Params variable as arguments. The IoT_Client_Connect_Params is optional after the first call to connect as the client retains the original values that were provided to support reconnect. The Connect params include values like Client Id, MQTT Version etc.
