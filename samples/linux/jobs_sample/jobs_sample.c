...
/**
 *
 * This example takes the parameters from the dsd_iot_config.h file and establishes
 * a connection to the DSD IoT MQTT Platform. It performs several operations to
 * demonstrate the basic capabilities of the DSD IoT Jobs platform.
 *
...
#include "../../../include/dsd_iot_jobs_interface.h"
#include "../../../include/dsd_iot_json_utils.h"
#include "../../../include/dsd_iot_log.h"
#include "../../../include/dsd_iot_mqtt_client_interface.h"
#include "../../../include/dsd_iot_version.h"
#include "dsd_iot_config.h"
...
/**
 * @brief Default MQTT HOST URL is pulled from the dsd_iot_config.h
 */
static char HostAddress[255] = DSD_IOT_MQTT_HOST;

/**
 * @brief Default MQTT port is pulled from the dsd_iot_config.h
 */
static uint32_t port = DSD_IOT_MQTT_PORT;

static jsmn_parser jsonParser;
static jsmntok_t jsonTokenStruct[MAX_JSON_TOKEN_EXPECTED];
static int32_t tokenCount;

static void iot_get_pending_callback_handler(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
...
static void iot_next_job_callback_handler(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
									IoT_Publish_Message_Params *params, void *pData) {
	...
	if (tokExecution) {
		...
		if (tok) {
			...
			rc = dsd_iot_jobs_send_update(pClient, QOS0, DSD_IOT_MY_THING_NAME, jobId, &updateRequest,
					topicToPublishUpdate, sizeof(topicToPublishUpdate), messageBuffer, sizeof(messageBuffer));
			if(SUCCESS != rc) {
				IOT_ERROR("dsd_iot_jobs_send_update returned error : %d ", rc);
				return;
			}
		}
	} else {
...
static void iot_update_accepted_callback_handler(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
									IoT_Publish_Message_Params *params, void *pData) {
	IOT_UNUSED(pData);
	IOT_UNUSED(pClient);
...
static void iot_update_rejected_callback_handler(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
									IoT_Publish_Message_Params *params, void *pData) {
	IOT_UNUSED(pData);
	IOT_UNUSED(pClient);
...
static void disconnectCallbackHandler(DSD_IoT_Client *pClient, void *data) {
	...
	if(dsd_iot_is_autoreconnect_enabled(pClient)) {
		IOT_INFO("Auto Reconnect is enabled, Reconnecting attempt will start now");
	} else {
		IOT_WARN("Auto Reconnect not enabled. Starting manual reconnect...");
		rc = dsd_iot_mqtt_attempt_reconnect(pClient);
...
int main(int argc, char **argv) {
	...
	IoT_Client_Connect_Params connectParams = dsd_init_conn_params(DSD_IOT_MY_USER_NAME, DSD_IOT_MY_PASSWORD);

	IoT_Publish_Message_Params paramsQOS0;

	getcwd(CurrentWD, sizeof(CurrentWD));
	snprintf(rootCA, PATH_MAX + 1, "%s/%s/%s", CurrentWD, certDirectory, DSD_IOT_ROOT_CA_FILENAME);
	snprintf(clientCRT, PATH_MAX + 1, "%s/%s/%s", CurrentWD, certDirectory, DSD_IOT_CERTIFICATE_FILENAME);
	snprintf(clientKey, PATH_MAX + 1, "%s/%s/%s", CurrentWD, certDirectory, DSD_IOT_PRIVATE_KEY_FILENAME);
	...
	rc = dsd_iot_mqtt_init(&client, &mqttInitParams, &connectParams);
	if(SUCCESS != rc) {
		IOT_ERROR("dsd_iot_mqtt_init returned error : %d ", rc);
		return rc;
	}
	...
	connectParams.pClientID = DSD_IOT_MQTT_CLIENT_ID;
	connectParams.clientIDLen = (uint16_t) strlen(DSD_IOT_MQTT_CLIENT_ID);
	connectParams.isWillMsgPresent = false;

	IOT_INFO("Connecting...");
	rc = dsd_iot_mqtt_connect(&client, &connectParams);
	if(SUCCESS != rc) {
		IOT_ERROR("Error(%d) connecting to %s:%d", rc, mqttInitParams.pHostURL, mqttInitParams.port);
		return rc;
	}
	/*
	 * Enable Auto Reconnect functionality. Minimum and Maximum time of Exponential backoff are set in dsd_iot_config.h
	 *  #DSD_IOT_MQTT_MIN_RECONNECT_WAIT_INTERVAL
	 *  #DSD_IOT_MQTT_MAX_RECONNECT_WAIT_INTERVAL
	 */
	rc = dsd_iot_mqtt_autoreconnect_set_status(&client, true);
	if(SUCCESS != rc) {
	...
	rc = dsd_iot_jobs_subscribe_to_job_messages(
		&client, QOS0, DSD_IOT_MY_THING_NAME, NULL, JOB_GET_PENDING_TOPIC, JOB_WILDCARD_REPLY_TYPE,
	...
	rc = dsd_iot_jobs_subscribe_to_job_messages(
		&client, QOS0, DSD_IOT_MY_THING_NAME, NULL, JOB_NOTIFY_NEXT_TOPIC, JOB_REQUEST_TYPE,
	...
	rc = dsd_iot_jobs_subscribe_to_job_messages(
		&client, QOS0, DSD_IOT_MY_THING_NAME, JOB_ID_NEXT, JOB_DESCRIBE_TOPIC, JOB_WILDCARD_REPLY_TYPE,
	...
	rc = dsd_iot_jobs_subscribe_to_job_messages(
		&client, QOS0, DSD_IOT_MY_THING_NAME, JOB_ID_WILDCARD, JOB_UPDATE_TOPIC, JOB_ACCEPTED_REPLY_TYPE,
	...
	rc = dsd_iot_jobs_subscribe_to_job_messages(
		&client, QOS0, DSD_IOT_MY_THING_NAME, JOB_ID_WILDCARD, JOB_UPDATE_TOPIC, JOB_REJECTED_REPLY_TYPE,
	...
	rc = dsd_iot_jobs_send_query(&client, QOS0, DSD_IOT_MY_THING_NAME, NULL, NULL, topicToPublishGetPending, sizeof(topicToPublishGetPending), NULL, 0, JOB_GET_PENDING_TOPIC);
	...
	rc = dsd_iot_jobs_describe(&client, QOS0, DSD_IOT_MY_THING_NAME, JOB_ID_NEXT, &describeRequest, topicToPublishGetNext, sizeof(topicToPublishGetNext), NULL, 0);

	while(SUCCESS == rc) {
		//Max time the yield function will wait for read messages
		rc = dsd_iot_mqtt_yield(&client, 50000);
	}
