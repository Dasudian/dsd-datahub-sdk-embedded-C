 ...
 * }
 *
 * This update message will remove the delta that was created. If this message was not removed then the DSD IoT Thing Shadow is going to always have a delta and keep sending delta any time an update is applied to the Shadow
 * This example will not use any of the json builder/helper functions provided in the dsd_iot_shadow_json_data.h.
 * @note Ensure the buffer sizes in dsd_iot_config.h are big enough to receive the delta message. The delta message will also contain the metadata with the timestamps
 */

static char certDirectory[PATH_MAX + 1] = "../../../certs";
#define HOST_ADDRESS_SIZE 255
static char HostAddress[HOST_ADDRESS_SIZE] = DSD_IOT_MQTT_HOST;
 ...
 * @note Any time messages are bigger than DSD_IOT_MQTT_RX_BUF_LEN the underlying MQTT library will ignore it. The maximum size of the message that can be received is limited to the DSD_IOT_MQTT_RX_BUF_LEN
 */
...
static bool buildJSONForReported(char *pJsonDocument, size_t maxSizeOfJsonDocument, const char *pReceivedDeltaData, uint32_t lengthDelta) {
	...
	if(dsd_iot_fill_with_client_token(tempClientTokenBuffer, MAX_SIZE_CLIENT_TOKEN_CLIENT_SEQUENCE) != SUCCESS){
		return false;
	}
...
int main(int argc, char** argv) {
	...
	DSD_IoT_Client mqttClient;
	IoT_Client_Connect_Params connectParams = dsd_init_shadow_conn_params(DSD_IOT_MY_USER_NAME, DSD_IOT_MY_PASSWORD, DSD_IOT_MQTT_CLIENT_ID);

	ShadowInitParameters_t sp = ShadowInitParametersDefault;
	sp.pHost = DSD_IOT_MQTT_HOST;
	sp.port = DSD_IOT_MQTT_PORT;
	...
	rc = dsd_iot_shadow_init(&mqttClient, &sp, &connectParams);
	if (SUCCESS != rc) {
	...
	scp.pMyThingName = DSD_IOT_MY_THING_NAME;
	scp.pMqttClientId = DSD_IOT_MQTT_CLIENT_ID;
	scp.mqttClientIdLen = (uint16_t) strlen(DSD_IOT_MQTT_CLIENT_ID);

	IOT_INFO("Shadow Connect");
	rc = dsd_iot_shadow_connect(&mqttClient, &scp, &connectParams);
	if (SUCCESS != rc) {
		IOT_ERROR("Shadow Connection Error");
		return rc;
	}
	 ...
	 */
	rc = dsd_iot_shadow_set_autoreconnect_status(&mqttClient, true);
	if(SUCCESS != rc){
	 ...
	 */
	rc = dsd_iot_shadow_register_delta(&mqttClient, &deltaObject);
	...
	while (NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc) {
		 ...
		 */
		rc = dsd_iot_shadow_yield(&mqttClient, 200);
		...
		if (messageArrivedOnDelta) {
			...
			rc = dsd_iot_shadow_update(&mqttClient, DSD_IOT_MY_THING_NAME, stringToEchoDelta, UpdateStatusCallback, NULL, 2, true);
	...
	rc = dsd_iot_shadow_disconnect(&mqttClient);
