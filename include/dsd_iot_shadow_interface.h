 ...
 */
#ifndef DSD_IOT_SDK_SRC_IOT_SHADOW_H_
#define DSD_IOT_SDK_SRC_IOT_SHADOW_H_
...
/**
 * @file dsd_iot_shadow_interface.h
 ...
 * 1. A MQTT Publish on the topic - $dsd/things/{thingName}/shadow/get
 * 2. Subscribe to MQTT topics - $dsd/things/{thingName}/shadow/get/accepted and $dsd/things/{thingName}/shadow/get/rejected.
 * If the request was successful we will receive the things json document in the accepted topic.
 *
 *
 */
#include "dsd_iot_mqtt_client_interface.h"
#include "dsd_iot_shadow_json_data.h"
...
typedef struct {
	...
	char *pClientCRT; ///< Location of Device certs signed by DSD IoT service
 ...
 * only for creating const variables. The client id will be assigned using the value from dsd_iot_config.h but the
...
/**
* @Init MQTT connection parameter with username and password as credentials
*
* @param username Reference to the Username for connecting to server as *char
* @param password Reference to the Password for connecting to server as *char
* @param clientID Reference to the clientID for connecting to server as *char
* @return IoT_Client_Connect_Params Type defining successful/failed API call
*/
...
*
* @param pClient MQTT Client that was previously created by calling dsd_iot_shadow_init
...
/**
 * @brief Connect to the DSD IoT Thing Shadow service over MQTT
 *
 ...
 * @param pConnParams   MQTT Connection parameters
 ...
 *
 * This function could be use in a separate thread waiting for the incoming messages, ensuring the connection is kept alive with the DSD Service.
...
/**
 * @brief Disconnect from the DSD IoT Thing Shadow service over MQTT
 *
 ...
 *
 * This function will be called from the context of \c dsd_iot_shadow_yield() context
 *
 ...
 * 1. Subscribe to Shadow topics - $dsd/things/{thingName}/shadow/update/accepted and $dsd/things/{thingName}/shadow/update/rejected
 * 2. wait for 2 seconds for the subscription to take effect
 * 3. Publish on the update topic - $dsd/things/{thingName}/shadow/update
 * 4. In the \c dsd_iot_shadow_yield() function the response will be handled. In case of timeout or if the response is received, the subscription to shadow response topics are un-subscribed from.
 ...
 * @param pJsonString The update action expects a JSON document to send. The JSON String should be a null terminated string. This JSON document should adhere to the DSD IoT Thing Shadow specification. To help in the process of creating this document- SDK provides apis in \c dsd_iot_shadow_json_data.h
 * @param callback This is the callback that will be used to inform the caller of the response from the DSD IoT Shadow service.Callback could be set to NULL if response is not important
...
/**
 * @brief This function is used to listen on the delta topic of #DSD_IOT_MY_THING_NAME mentioned in the dsd_iot_config.h file.
 *
 ...
 */
IoT_Error_t dsd_iot_shadow_register_delta(DSD_IoT_Client *pClient, jsonStruct_t *pStruct);
...
/**
 * @brief Version of a document is received with every accepted/rejected and the SDK keeps track of the last received version of the JSON document of #DSD_IOT_MY_THING_NAME shadow
 *
...
#endif //DSD_IOT_SDK_SRC_IOT_SHADOW_H_
