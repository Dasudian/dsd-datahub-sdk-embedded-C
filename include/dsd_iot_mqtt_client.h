...
/**
 * @file dsd_iot_mqtt_client.h
 * @brief Client definition for MQTT
 */

#ifndef DSD_IOT_SDK_SRC_IOT_MQTT_CLIENT_H
#define DSD_IOT_SDK_SRC_IOT_MQTT_CLIENT_H
...
/* DSD Specific header files */
#include "dsd_iot_error.h"
...
typedef struct _Client DSD_IoT_Client;

/**
 * @brief Quality of Service Type
 *
 * Defining a QoS type.
 * @note QoS 2 is \b NOT supported by the DSD IoT Service at the time of this SDK release.
 *
 */
typedef enum QoS {
	QOS0 = 0,
...
typedef struct {
	...
	uint8_t isRetained;	///< Retained messages are \b NOT supported by the DSD IoT Service at the time of this SDK release.
 ...
 * @note Retained messages are \b NOT supported by the DSD IoT Service at the time of this SDK release.
 *
 */
typedef struct {
...
typedef struct {
	...
	char *pClientID;                	///< Pointer to a string defining the MQTT client ID (this needs to be unique \b per \b device across your DSD account)
	...
	char *pUsername;			///< Not used in the DSD IoT Service, will need to be cstring if used
	uint16_t usernameLen;			///< Username Length. 16 bit unsigned integer
	char *pPassword;			///< Not used in the DSD IoT Service, will need to be cstring if used
 ...
 *
 */
typedef void (*iot_disconnect_handler)(DSD_IoT_Client *, void *);
 ...
 *
 */
typedef void (*pApplicationHandler_t)(DSD_IoT_Client *pClient, char *pTopicName, uint16_t topicNameLen,
...
typedef struct _ClientData {
	...
	size_t readBufSize;
    size_t readBufIndex;
	unsigned char writeBuf[DSD_IOT_MQTT_TX_BUF_LEN];
	unsigned char readBuf[DSD_IOT_MQTT_RX_BUF_LEN];
	...
	MessageHandlers messageHandlers[DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS];
