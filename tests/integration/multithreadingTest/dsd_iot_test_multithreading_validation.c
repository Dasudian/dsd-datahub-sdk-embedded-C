...
#include "../include/dsd_iot_config.h"
#include "../include/dsd_iot_integ_tests_config.h"
...
static void dsd_iot_mqtt_tests_message_aggregator(DSD_IoT_Client *pClient, char *topicName,
...
static void dsd_iot_mqtt_tests_disconnect_callback_handler(DSD_IoT_Client *pClient, void *param) {
	IOT_UNUSED(pClient);
	IOT_UNUSED(param);
}

static IoT_Error_t dsd_iot_mqtt_tests_subscribe_to_test_topic(DSD_IoT_Client *pClient, QoS qos, struct timeval *pSubscribeTime) {
	IoT_Error_t rc = SUCCESS;
	struct timeval start, end;

	gettimeofday(&start, NULL);
	rc = dsd_iot_mqtt_subscribe(pClient, INTEGRATION_TEST_TOPIC, strlen(INTEGRATION_TEST_TOPIC), qos,
								dsd_iot_mqtt_tests_message_aggregator, NULL);
...
static void *dsd_iot_mqtt_tests_yield_thread_runner(void *ptr) {
	IoT_Error_t rc = SUCCESS;
	DSD_IoT_Client *pClient = (DSD_IoT_Client *) ptr;
	while(SUCCESS == rc && false == terminate_yield_thread) {
		do {
			usleep(THREAD_SLEEP_INTERVAL_USEC);
			//DEBUG("\n Yielding \n");
			rc = dsd_iot_mqtt_yield(pClient, 100);
...
static void *dsd_iot_mqtt_tests_publish_thread_runner(void *ptr) {
	int itr = 0;
	char cPayload[100];
	IoT_Publish_Message_Params params;
	IoT_Error_t rc = SUCCESS;
	ThreadData *threadData = (ThreadData *) ptr;
	DSD_IoT_Client *pClient = threadData->client;
	int threadId = threadData->threadId;

	for(itr = 0; itr < PUBLISH_COUNT; itr++) {
		snprintf(cPayload, 100, "%s_Thread : %d, Msg : %d", DSD_IOT_MY_THING_NAME, threadId, itr);
		...
		do {
			rc = dsd_iot_mqtt_publish(pClient, INTEGRATION_TEST_TOPIC, strlen(INTEGRATION_TEST_TOPIC), &params);
			usleep(THREAD_SLEEP_INTERVAL_USEC);
		} while(MUTEX_LOCK_ERROR == rc || MQTT_CLIENT_NOT_IDLE_ERROR == rc);
		if(SUCCESS != rc) {
			IOT_WARN("\nFailed attempt 1 Publishing Thread : %d, Msg : %d, cs : %d --> %d\n ", threadId, itr, rc,
				 dsd_iot_mqtt_get_client_state(pClient));
			do {
				rc = dsd_iot_mqtt_publish(pClient, INTEGRATION_TEST_TOPIC, strlen(INTEGRATION_TEST_TOPIC), &params);
				usleep(THREAD_SLEEP_INTERVAL_USEC);
			} while(MUTEX_LOCK_ERROR == rc || MQTT_CLIENT_NOT_IDLE_ERROR == rc);
			...
			if(SUCCESS != rc) {
				IOT_ERROR("\nFailed attempt 2 Publishing Thread : %d, Msg : %d, cs : %d --> %d Second Attempt \n", threadId,
					  itr, rc, dsd_iot_mqtt_get_client_state(pClient));
			}
		}
	}
...
static void *dsd_iot_mqtt_tests_sub_unsub_thread_runner(void *ptr) {
	IoT_Error_t rc = SUCCESS;
	DSD_IoT_Client *pClient = (DSD_IoT_Client *) ptr;
	char testTopic[50];
	...
	while(SUCCESS == rc && false == terminate_subUnsub_thread) {
		do {
			usleep(THREAD_SLEEP_INTERVAL_USEC);
			rc = dsd_iot_mqtt_subscribe(pClient, testTopic, strlen(testTopic), QOS1,
										dsd_iot_mqtt_tests_message_aggregator, NULL);
		} while(MQTT_CLIENT_NOT_IDLE_ERROR == rc);
		...
		do {
			usleep(THREAD_SLEEP_INTERVAL_USEC);
			rc = dsd_iot_mqtt_unsubscribe(pClient, testTopic, strlen(testTopic));
		} while(MQTT_CLIENT_NOT_IDLE_ERROR == rc);
...
int dsd_iot_mqtt_tests_multi_threading_validation() {
	...
	do {
		...
		snprintf(root_CA, PATH_MAX + 1, "%s/%s/%s", CurrentWD, certDirectory, DSD_IOT_ROOT_CA_FILENAME);
		...
		initParams.pHostURL = DSD_IOT_MQTT_HOST;
		initParams.port =  DSD_IOT_MQTT_PORT;
		...
		initParams.disconnectHandler = dsd_iot_mqtt_tests_disconnect_callback_handler;
		initParams.enableAutoReconnect = false;
		initParams.isBlockOnThreadLockEnabled = true;
		dsd_iot_mqtt_init(&client, &initParams);
	...
	dsd_iot_mqtt_tests_subscribe_to_test_topic(&client, QOS1, &subscribeTopic);

	printf("\nRunning Test! ");

	yieldThreadReturn = pthread_create(&yield_thread, NULL, dsd_iot_mqtt_tests_yield_thread_runner, &client);
	subUnsubThreadReturn = pthread_create(&sub_unsub_thread, NULL, dsd_iot_mqtt_tests_sub_unsub_thread_runner, &client);

	for(i = 0; i < MAX_PUB_THREAD_COUNT; i++) {
		threadData[i].client = &client;
		threadData[i].threadId = threadId[i];
		pubThreadReturn[i] = pthread_create(&publish_thread[i], NULL, dsd_iot_mqtt_tests_publish_thread_runner,
											&threadData[i]);
	}
	...
	dsd_iot_mqtt_disconnect(&client);
	return test_result;
}
...
int main() {
	...
	printf("******************************************************************\n");
	int rc = dsd_iot_mqtt_tests_multi_threading_validation();
	if(0 != rc) {
		printf("\n*******************************************************************\n");
