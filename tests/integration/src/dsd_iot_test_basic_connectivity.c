...
/**
 * @file dsd_iot_test_basic_connectivity.c
...
static IoT_Error_t dsd_iot_mqtt_tests_subscribe_to_test_topic(DSD_IoT_Client *pClient, QoS qos,
...
static void *dsd_iot_mqtt_tests_publish_thread_runner(void *ptr) {
	...
	for(i = 0; i < PUBLISH_COUNT; i++) {
		snprintf(cPayload, 100, "%s_Thread : %d, Msg : %d", DSD_IOT_MY_THING_NAME, threadId, i + 1);
...
int dsd_iot_mqtt_tests_basic_connectivity() {
	...
	pubThreadReturn = pthread_create(&publish_thread, NULL, dsd_iot_mqtt_tests_publish_thread_runner, &threadData);
