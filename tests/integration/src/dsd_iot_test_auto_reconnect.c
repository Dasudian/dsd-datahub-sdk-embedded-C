...
/**
 * @file dsd_iot_test_auto_reconnect.c
 ...
 */
void dsd_iot_mqtt_tests_block_tls_connect(DSD_IoT_Client *pClient) {
 ...
 */
void dsd_iot_mqtt_tests_unblock_tls_connect(DSD_IoT_Client *pClient) {
	pClient->networkStack.tlsConnectParams.pRootCALocation = savedLocation;
}

void *dsd_iot_mqtt_tests_yield_with_rc(void *ptr) {
	...
	while(terminate_yield_with_rc_thread == false
		...
		usleep(500000);
		printf(" Client state : %d ", dsd_iot_mqtt_get_client_state(pClient));
...
void dsd_iot_mqtt_tests_disconnect_callback_handler(DSD_IoT_Client *pClient, void *param) {
	disconnectedCounter++;
}

int dsd_iot_mqtt_tests_auto_reconnect() {
	...
	yieldThreadReturn = pthread_create(&yield_thread, NULL, dsd_iot_mqtt_tests_yield_with_rc, &client);

	/*
	 * Test disconnect handler
	 */
	printf("1. Test Disconnect Handler\n");
	dsd_iot_mqtt_tests_block_tls_connect(&client);
	...
	if(disconnectedCounter == 1) {
		...
		dsd_iot_mqtt_tests_unblock_tls_connect(&client);
	 ...
	 */
	printf("2. Test Manual Reconnect, Current Client state : %d \n", dsd_iot_mqtt_get_client_state(&client));
	rc = dsd_iot_mqtt_attempt_reconnect(&client);
	...
	if(true == dsd_iot_mqtt_is_client_connected(&client)) {
	...
	rc = dsd_iot_mqtt_disconnect(&client);
	return rc;
}
