...
/**
 * @file dsd_iot_test_multiple_clients.c
 * @brief Integration Test for multiple clients from the same application
 */

#include "../include/dsd_iot_test_integration_common.h"
...
static void dsd_iot_mqtt_tests_message_aggregator(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
...
static IoT_Error_t dsd_iot_mqtt_tests_connect_client_to_service(DSD_IoT_Client *pClient, struct timeval *pConnectTime,
	...
	rc = dsd_iot_mqtt_init(pClient, &initParams);
	printf("\n Init response : %d", rc);
	...
	rc = dsd_iot_mqtt_connect(pClient, &connectParams);
...
int dsd_iot_mqtt_tests_multiple_clients() {
	...
	DSD_IoT_Client pubClient;
	DSD_IoT_Client subClient;
	...
	do {
		rc = dsd_iot_mqtt_tests_connect_client_to_service(&pubClient, &connectTime, pubClientId, rootCA,
														  clientCRT, clientKey);
		connectCounter++;
	...
	do {
		rc = dsd_iot_mqtt_tests_connect_client_to_service(&subClient, &connectTime, subClientId, rootCA,
														  clientCRT, clientKey);
		connectCounter++;
	...
	dsd_iot_mqtt_tests_subscribe_to_test_topic(&subClient, QOS1, &subscribeTopic);

	yieldThreadReturn = pthread_create(&yield_thread, NULL, dsd_iot_mqtt_tests_yield_thread_runner, &subClient);
	pubThreadReturn = pthread_create(&publish_thread, NULL, dsd_iot_mqtt_tests_publish_thread_runner, &pubClient);
	...
	dsd_iot_mqtt_disconnect(&pubClient);
	dsd_iot_mqtt_disconnect(&subClient);
