...
void iot_get_pending_callback_handler(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
...
void iot_next_job_callback_handler(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
									IoT_Publish_Message_Params *params, void *pData) {
...
int dsd_iot_jobs_basic_test() {
	...
	gettimeofday(&start, NULL);
	while (!callbackDone) {
		dsd_iot_mqtt_yield(&client, 5000);
