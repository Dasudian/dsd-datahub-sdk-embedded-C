...
/**
 * @file dsd_iot_tests_unit_yield_helper.c
...
static void iot_tests_unit_acr_subscribe_callback_handler(DSD_IoT_Client *pClient, char *topicName,
...
static void iot_tests_unit_yield_test_subscribe_callback_handler(DSD_IoT_Client *pClient, char *topicName,
																 uint16_t topicNameLen,
																 IoT_Publish_Message_Params *params, void *pData) {
	char *tmp = params->payload;
	unsigned int i;
	...
	rc = dsd_iot_mqtt_yield(pClient, 1000);
...
void iot_tests_unit_disconnect_handler(DSD_IoT_Client *pClient, void *disconParam) {
	IOT_UNUSED(pClient);
...
TEST_GROUP_C_SETUP(YieldTests) {
	...
	InitMQTTParamsSetup(&initParams, DSD_IOT_MQTT_HOST, DSD_IOT_MQTT_PORT, false, iot_tests_unit_disconnect_handler);
	rc = dsd_iot_mqtt_init(&iotClient, &initParams);
	CHECK_EQUAL_C_INT(SUCCESS, rc);


	ConnectMQTTParamsSetup_Detailed(&connectParams, DSD_IOT_MQTT_CLIENT_ID, (uint16_t) strlen(DSD_IOT_MQTT_CLIENT_ID),
	...
	rc = dsd_iot_mqtt_autoreconnect_set_status(&iotClient, false);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
	ResetTLSBuffer();
}
...
TEST_C(YieldTests, NullClientYield) {
	IoT_Error_t rc = dsd_iot_mqtt_yield(NULL, 1000);
...
TEST_C(YieldTests, ZeroTimeoutYield) {
	IoT_Error_t rc = dsd_iot_mqtt_yield(&iotClient, 0);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}
/* G:3 - Yield, network disconnected, never connected */

TEST_C(YieldTests, YieldNetworkDisconnectedNeverConnected) {
	DSD_IoT_Client tempIotClient;
	IoT_Error_t rc;

	InitMQTTParamsSetup(&initParams, DSD_IOT_MQTT_HOST, DSD_IOT_MQTT_PORT, false, iot_tests_unit_disconnect_handler);
	rc = dsd_iot_mqtt_init(&tempIotClient, &initParams);
	CHECK_EQUAL_C_INT(SUCCESS, rc);

	rc = dsd_iot_mqtt_yield(&tempIotClient, 1000);
...
TEST_C(YieldTests, YieldInSubscribeCallback) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, subTopic, subTopicLen, QOS1,
...
TEST_C(YieldTests, disconnectNoAutoReconnect) {
	...
	rc = dsd_iot_mqtt_autoreconnect_set_status(&iotClient, true);
	CHECK_EQUAL_C_INT(SUCCESS, rc);

	CHECK_EQUAL_C_INT(true, dsd_iot_mqtt_is_client_connected(&iotClient));
	CHECK_EQUAL_C_INT(true, dsd_iot_is_autoreconnect_enabled(&iotClient));

	/* Disable Autoreconnect, then let ping request time out and call yield */
	dsd_iot_mqtt_autoreconnect_set_status(&iotClient, false);
	...
	rc = dsd_iot_mqtt_yield(&iotClient, 100);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
	...
	CHECK_EQUAL_C_INT(0, dsd_iot_mqtt_is_client_connected(&iotClient));
...
TEST_C(YieldTests, disconnectManualAutoReconnect) {
	...
	CHECK_C(dsd_iot_mqtt_is_client_connected(&iotClient));

	/* Disable Autoreconnect, then let ping request time out and call yield */
	dsd_iot_mqtt_autoreconnect_set_status(&iotClient, false);
	CHECK_C(!dsd_iot_is_autoreconnect_enabled(&iotClient));
	...
	dcHandlerInvoked = false;
	setTLSRxBufferForConnack(&connectParams, 0, 0);
	rc = dsd_iot_mqtt_attempt_reconnect(&iotClient);
...
TEST_C(YieldTests, resubscribeSuccessfulReconnect) {
	...
	connected = dsd_iot_mqtt_is_client_connected(&iotClient);
	...
	setTLSRxBufferForSuback(subTopic, subTopicLen, QOS1, testPubMsgParams);
	rc = dsd_iot_mqtt_subscribe(&iotClient, subTopic, subTopicLen, QOS0, iot_tests_unit_acr_subscribe_callback_handler,
								NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
	...
	autoReconnectEnabled = dsd_iot_is_autoreconnect_enabled(&iotClient);
