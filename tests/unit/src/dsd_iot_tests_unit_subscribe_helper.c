...
/**
 * @file dsd_iot_tests_unit_subscribe_helper.c
...
static DSD_IoT_Client iotClient;
...
static void iot_subscribe_callback_handler(DSD_IoT_Client *pClient, char *topicName,
...
static void iot_subscribe_callback_handler1(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
...
static void iot_subscribe_callback_handler2(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
											IoT_Publish_Message_Params *params, void *pData) {
	if(NULL == pClient || NULL == topicName || 0 == topicNameLen) {
		return;
	}
...
static void iot_subscribe_callback_handler3(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
											IoT_Publish_Message_Params *params, void *pData) {
	if(NULL == pClient || NULL == topicName || 0 == topicNameLen) {
		return;
	}
...
static void iot_subscribe_callback_handler4(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
											IoT_Publish_Message_Params *params, void *pData) {
	if(NULL == pClient || NULL == topicName || 0 == topicNameLen) {
		return;
	}
...
static void iot_subscribe_callback_handler5(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
											IoT_Publish_Message_Params *params, void *pData) {
	if(NULL == pClient || NULL == topicName || 0 == topicNameLen) {
		return;
	}
...
static void iot_subscribe_callback_handler6(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
											IoT_Publish_Message_Params *params, void *pData) {
	if(NULL == pClient || NULL == topicName || 0 == topicNameLen) {
		return;
	}
...
TEST_GROUP_C_SETUP(SubscribeTests) {
	IoT_Error_t rc;
	ResetTLSBuffer();
	InitMQTTParamsSetup(&initParams, DSD_IOT_MQTT_HOST, DSD_IOT_MQTT_PORT, false, NULL);
	initParams.mqttCommandTimeout_ms = 2000;
	rc = dsd_iot_mqtt_init(&iotClient, &initParams);
	...
	rc = dsd_iot_mqtt_connect(&iotClient, &connectParams);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
	IOT_DEBUG("MQTT Status State : %d, RC : %d\n\n", dsd_iot_mqtt_get_client_state(&iotClient), rc);
...
TEST_C(SubscribeTests, SubscribeNullClient) {
	IoT_Error_t rc = dsd_iot_mqtt_subscribe(NULL, "sdkTest/Sub", 11, QOS1, iot_subscribe_callback_handler, &iotClient);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

/* C:2 - Subscribe with Null/empty Topic Name */
TEST_C(SubscribeTests, SubscribeNullTopic) {
	IoT_Error_t rc = dsd_iot_mqtt_subscribe(&iotClient, NULL, 11, QOS1, iot_subscribe_callback_handler, &iotClient);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

/* C:3 - Subscribe with Null client callback */
TEST_C(SubscribeTests, SubscribeNullSubscribeHandler) {
	IoT_Error_t rc = dsd_iot_mqtt_subscribe(&iotClient, "sdkTest/Sub", 11, QOS1, NULL, &iotClient);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}
...
TEST_C(SubscribeTests, SubscribeNullSubscribeHandlerData) {
	...
	IoT_Error_t rc = dsd_iot_mqtt_subscribe(&iotClient, "sdkTest/Sub", 11, QOS1, iot_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
}
/* C:5 - Subscribe with no connection */
TEST_C(SubscribeTests, SubscribeNoConnection) {
	/* Disconnect first */
	IoT_Error_t rc = dsd_iot_mqtt_disconnect(&iotClient);

	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdkTest/Sub", 11, QOS1, iot_subscribe_callback_handler, NULL);
...
TEST_C(SubscribeTests, subscribeQoS0FailureOnNoSuback) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, subTopic, subTopicLen, QOS0, iot_subscribe_callback_handler, NULL);
...
TEST_C(SubscribeTests, subscribeQoS1FailureOnNoSuback) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, subTopic, subTopicLen, QOS1, iot_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(MQTT_REQUEST_TIMEOUT_ERROR, rc);
...
TEST_C(SubscribeTests, subscribeQoS0MsgReceivedAndNoPubackSent) {
	...
	if(SUCCESS == rc) {
		...
		rc = dsd_iot_mqtt_yield(&iotClient, 1000);
		if(SUCCESS == rc) {
...
TEST_C(SubscribeTests, SubscribeToMultipleTopicsSuccess) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/Test1", 9, QOS1, iot_subscribe_callback_handler1, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);

	setTLSRxBufferForSuback("sdk/Test2", 9, QOS1, testPubMsgParams);
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/Test2", 9, QOS1, iot_subscribe_callback_handler2, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
...
TEST_C(SubscribeTests, SubcribeToMaxAllowedTopicsSuccess) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/Test3", 9, QOS1, iot_subscribe_callback_handler3, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);

	setTLSRxBufferForSuback("sdk/Test4", 9, QOS1, testPubMsgParams);
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/Test4", 9, QOS1, iot_subscribe_callback_handler4, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);

	setTLSRxBufferForSuback("sdk/Test5", 9, QOS1, testPubMsgParams);
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/Test5", 9, QOS1, iot_subscribe_callback_handler5, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
...
TEST_C(SubscribeTests, SubcribeToMaxPlusOneAllowedTopicsFailure) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/Test6", 9, QOS1, iot_subscribe_callback_handler6, NULL);
...
TEST_C(SubscribeTests, subscribeTopicWithHashkeyAllSubTopicSuccess) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/Test/#", strlen("sdk/Test/#"), QOS1,
...
TEST_C(SubscribeTests, subscribeTopicHashkeyMustBeTheLastFail) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/#/sub", strlen("sdk/#/sub"), QOS1,
								iot_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
...
TEST_C(SubscribeTests, subscribeTopicWithPluskeySuccess) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, "sdk/Test/+/sub", strlen("sdk/Test/+/sub"), QOS1,
								iot_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
