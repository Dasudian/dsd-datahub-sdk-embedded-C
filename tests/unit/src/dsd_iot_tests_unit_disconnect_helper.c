...
/**
 * @file dsd_iot_tests_unit_disconnect_helper.c
...
void disconnectTestHandler(DSD_IoT_Client *pClient, void *disconHandlerParam) {
	IOT_UNUSED(pClient);
...
TEST_GROUP_C_SETUP(DisconnectTests) {
	IoT_Error_t rc;
	ResetTLSBuffer();
	InitMQTTParamsSetup(&initParams, DSD_IOT_MQTT_HOST, DSD_IOT_MQTT_PORT, true, disconnectTestHandler);
...
TEST_C(DisconnectTests, NullClientDisconnect) {
	IoT_Error_t rc = dsd_iot_mqtt_disconnect(NULL);
...
TEST_C(DisconnectTests, NullClientSetDisconnectHandler) {
	IoT_Error_t rc = dsd_iot_mqtt_set_disconnect_handler(NULL, disconnectTestHandler, NULL);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

/* F:3 - Call Set Disconnect handler with Null handler */
TEST_C(DisconnectTests, SetDisconnectHandlerNullHandler) {
	IoT_Error_t rc = dsd_iot_mqtt_set_disconnect_handler(&iotClient, NULL, NULL);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}
...
TEST_C(DisconnectTests, disconnectNotConnected) {
	...
	CHECK_EQUAL_C_INT(false, dsd_iot_mqtt_is_client_connected(&iotClient));
...
TEST_C(DisconnectTests, HandlerInvokedOnDisconnect) {
	...
	currentAutoReconnectStatus = dsd_iot_is_autoreconnect_enabled(&iotClient);
	...
	dcCount = dsd_iot_mqtt_get_network_disconnected_count(&iotClient);
	CHECK_C(1 == dcCount);

	dsd_iot_mqtt_reset_network_disconnected_count(&iotClient);

	dcCount = dsd_iot_mqtt_get_network_disconnected_count(&iotClient);
	CHECK_C(0 == dcCount);

	ResetTLSBuffer();
	dsd_iot_mqtt_autoreconnect_set_status(&iotClient, currentAutoReconnectStatus);
...
TEST_C(DisconnectTests, SetHandlerAndInvokedOnDisconnect) {
	...
	handlerInvoked = false;
	InitMQTTParamsSetup(&initParams, "localhost", DSD_IOT_MQTT_PORT, false, NULL);
	rc = dsd_iot_mqtt_init(&iotClient, &initParams);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
	...
	dsd_iot_mqtt_set_disconnect_handler(&iotClient, disconnectTestHandler, NULL);
	dsd_iot_mqtt_autoreconnect_set_status(&iotClient, true);
