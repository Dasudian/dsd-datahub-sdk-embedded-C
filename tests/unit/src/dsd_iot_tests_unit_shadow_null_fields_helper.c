...
/**
 * @file dsd_iot_tests_unit_shadow_null_fields_helper.c
...
TEST_C(ShadowNullFields, NullHost) {
	...
	IoT_Error_t rc = dsd_iot_shadow_init(&client, &shadowInitParams);
...
TEST_C(ShadowNullFields, NullClientID) {
	...
	rc = dsd_iot_shadow_connect(&client, &shadowConnectParams);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}
...
TEST_C(ShadowNullFields, NullClientInit) {
	...
	shadowInitParams.port = DSD_IOT_MQTT_PORT;
	shadowInitParams.disconnectHandler = NULL;
	IoT_Error_t rc = dsd_iot_shadow_init(NULL, &shadowInitParams);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}
...
TEST_C(ShadowNullFields, NullClientConnect) {
	...
	rc = dsd_iot_shadow_connect(NULL, &shadowConnectParams);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

TEST_C(ShadowNullFields, NullUpdateDocument) {
	IoT_Error_t rc = dsd_iot_shadow_internal_action(DSD_IOT_MY_THING_NAME, SHADOW_UPDATE, NULL, 0, actionCallbackNullTest,
													NULL, 4, false);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

TEST_C(ShadowNullFields, NullClientYield) {
	IoT_Error_t rc = dsd_iot_shadow_yield(NULL, 1000);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

TEST_C(ShadowNullFields, NullClientDisconnect) {
	IoT_Error_t rc = dsd_iot_shadow_disconnect(NULL);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

TEST_C(ShadowNullFields, NullClientShadowGet) {
	IoT_Error_t rc = dsd_iot_shadow_get(NULL, DSD_IOT_MY_THING_NAME, actionCallbackNullTest, NULL, 100, true);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

TEST_C(ShadowNullFields, NullClientShadowUpdate) {
	IoT_Error_t rc = dsd_iot_shadow_update(NULL, DSD_IOT_MY_THING_NAME, jsonFullDocument,
										   actionCallbackNullTest, NULL, 100, true);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

TEST_C(ShadowNullFields, NullClientShadowDelete) {
	IoT_Error_t rc = dsd_iot_shadow_delete(NULL, DSD_IOT_MY_THING_NAME, actionCallbackNullTest, NULL, 100, true);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

TEST_C(ShadowNullFields, NullClientSetAutoreconnect) {
	IoT_Error_t rc = dsd_iot_shadow_set_autoreconnect_status(NULL, true);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}
