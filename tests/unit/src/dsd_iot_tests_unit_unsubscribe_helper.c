...
/**
 * @file dsd_iot_tests_unit_unsubscribe_helper.c
...
static void iot_subscribe_callback_handler(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
...
TEST_C(UnsubscribeTests, UnsubscribeNullClient) {
	IoT_Error_t rc = dsd_iot_mqtt_unsubscribe(NULL, "sdkTest/Sub", 11);
...
TEST_C(UnsubscribeTests, UnsubscribeNullTopic) {
	IoT_Error_t rc = dsd_iot_mqtt_unsubscribe(&iotClient, NULL, 11);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
}

/* D:3 - Unsubscribe, Not subscribed to topic */
TEST_C(UnsubscribeTests, UnsubscribeNotSubscribed) {
	IoT_Error_t rc = dsd_iot_mqtt_unsubscribe(&iotClient, "sdkTest/Sub", 11);
...
TEST_C(UnsubscribeTests, unsubscribeQoS0FailureOnNoUnsuback) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, subTopic, subTopicLen, QOS0, iot_subscribe_callback_handler,
								NULL);
	if(SUCCESS == rc) {
		// Then, unsubscribe
		rc = dsd_iot_mqtt_unsubscribe(&iotClient, subTopic, (uint16_t) strlen(subTopic));
...
TEST_C(UnsubscribeTests, unsubscribeQoS1FailureOnNoUnsuback) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, subTopic, subTopicLen, QOS1, iot_subscribe_callback_handler,
								NULL);
	if(SUCCESS == rc) {
		// Then, unsubscribe
		rc = dsd_iot_mqtt_unsubscribe(&iotClient, subTopic, (uint16_t) strlen(subTopic));
...
TEST_C(UnsubscribeTests, MsgAfterUnsubscribe) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, "topic1", 6, QOS0, iot_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
	...
	//3.
	setTLSRxBufferForUnsuback();
	rc = dsd_iot_mqtt_unsubscribe(&iotClient, "topic1", 6);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
	//reset the string
...
TEST_C(UnsubscribeTests, MaxTopicsSubscription) {
	IoT_Error_t rc = SUCCESS;
	int i = 0;
	char topics[DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS + 1][10];
	...
	for(i = 0; i < DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS; i++) {
		...
		rc = dsd_iot_mqtt_subscribe(&iotClient, topics[i], (uint16_t) strlen(topics[i]), QOS0, iot_subscribe_callback_handler,
									NULL);
		CHECK_EQUAL_C_INT(SUCCESS, rc);
	}
	snprintf(topics[i], 10, "topic-%d", i);
	setTLSRxBufferForSuback(topics[i], strlen(topics[i]), QOS0, testPubMsgParams);
	rc = dsd_iot_mqtt_subscribe(&iotClient, topics[i], (uint16_t) strlen(topics[i]), QOS0, iot_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(MQTT_MAX_SUBSCRIPTIONS_REACHED_ERROR, rc);

	// 2.
	setTLSRxBufferForUnsuback();
	rc = dsd_iot_mqtt_unsubscribe(&iotClient, topics[i - 1], (uint16_t) strlen(topics[i - 1]));
	CHECK_EQUAL_C_INT(SUCCESS, rc);
...
TEST_C(UnsubscribeTests, RepeatedSubUnSub) {
	...
	int i = 0;
	char expectedCallbackString[100];
	char topics[3 * DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS][10];

	IOT_DEBUG("-->Running Unsubscribe Tests - D:12 - Repeated Subscribe and Unsubscribe \n");


	for(i = 0; i < 3 * DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS; i++) {
		//1.
		snprintf(topics[i], 10, "topic-%d", i);
		setTLSRxBufferForSuback(topics[i], 10, QOS0, testPubMsgParams);
		rc = dsd_iot_mqtt_subscribe(&iotClient, topics[i], 10, QOS0, iot_subscribe_callback_handler, NULL);
		CHECK_EQUAL_C_INT(SUCCESS, rc);
		...
		//2.
		setTLSRxBufferForUnsuback();
		rc = dsd_iot_mqtt_unsubscribe(&iotClient, topics[i], (uint16_t) strlen(topics[i]));
		CHECK_EQUAL_C_INT(SUCCESS, rc);
	}
