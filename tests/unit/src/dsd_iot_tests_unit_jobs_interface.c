...
TEST_GROUP_C_SETUP(JobsInterfaceTest) {
	...
	InitMQTTParamsSetup(&mqttInitParams, DSD_IOT_MQTT_HOST, DSD_IOT_MQTT_PORT, false, NULL);
	ret_val = dsd_iot_mqtt_init(&client, &mqttInitParams);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);

	ConnectMQTTParamsSetup(&connectParams, (char *) DSD_IOT_MQTT_CLIENT_ID, (uint16_t) strlen(DSD_IOT_MQTT_CLIENT_ID));
	setTLSRxBufferForConnack(&connectParams, 0, 0);
	ret_val = dsd_iot_mqtt_connect(&client, &connectParams);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
...
TEST_GROUP_C_TEARDOWN(JobsInterfaceTest) {
	IoT_Error_t rc = dsd_iot_mqtt_disconnect(&client);
	IOT_UNUSED(rc);
}
...
static void publishTestMessage(AwsIotJobExecutionTopicType topicType, bool withJobId) {
	...
	int replyTopicLen = dsd_iot_jobs_get_api_topic(
	...
	IoT_Error_t error = dsd_iot_mqtt_yield(&client, 100);
...
static void testSubscribeAndUnsubscribe(AwsIotJobExecutionTopicType topicType, bool withJobId) {
	...
	int expectedTopicLength = dsd_iot_jobs_get_api_topic(
	...
	if (topicType == JOB_WILDCARD_TOPIC && !withJobId) {
		iotError = dsd_iot_jobs_subscribe_to_all_job_messages(
				&client, QOS0, THING_NAME, testCallback, &CALLBACK_DATA, topicBuffer, sizeof(topicBuffer));
	} else {
		iotError = dsd_iot_jobs_subscribe_to_job_messages(
	...
	iotError = dsd_iot_jobs_unsubscribe_from_job_messages(&client, topicBuffer);
...
static void testSendQuery(bool withJobId, bool withClientToken, AwsIotJobExecutionTopicType topicType) {
	...
	if (topicType == JOB_DESCRIBE_TOPIC) {
		...
		iotError = dsd_iot_jobs_describe(
...
TEST_C(JobsInterfaceTest, TestSendUpdate) {
	...
	char expectedTopic[MAX_JOB_TOPIC_LENGTH_BYTES + 1];
	char topicBuffer[MAX_JOB_TOPIC_LENGTH_BYTES + 1];
	char messageBuffer[DSD_IOT_MQTT_TX_BUF_LEN + 1];
	...
	setTLSRxBufferForPuback();
	IoT_Error_t iotError;
	iotError = dsd_iot_jobs_send_update(
