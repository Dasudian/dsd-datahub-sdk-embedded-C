...
/**
 * @file dsd_iot_tests_unit_shadow_delta_helper.c
...
#define SHADOW_DELTA_UPDATE "$dsd/things/%s/shadow/update/delta"

#undef DSD_IOT_MY_THING_NAME
#define DSD_IOT_MY_THING_NAME "DSD-IoT-C-SDK"
...
TEST_GROUP_C_SETUP(ShadowDeltaTest) {
	...
	snprintf(shadowDeltaTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, SHADOW_DELTA_UPDATE, DSD_IOT_MY_THING_NAME);
}
...
TEST_C(ShadowDeltaTest, registerDeltaInt) {
	...
	ret_val = dsd_iot_shadow_register_delta(&client, &intHandler);

	ResetTLSBuffer();
	setTLSRxBufferWithMsgOnSubscribedTopic(shadowDeltaTopic, strlen(shadowDeltaTopic), QOS0, params, params.payload);

	dsd_iot_shadow_yield(&client, 3000);
...
TEST_C(ShadowDeltaTest, DeltaNestedObject) {
	...
	ret_val = dsd_iot_shadow_register_delta(&client, &nestedObjectHandler);
...
TEST_C(ShadowDeltaTest, DeltaVersionIgnoreOldVersion) {
	...
	dsd_iot_shadow_yield(&client, 100);
	CHECK_EQUAL_C_STRING(sentNestedObjectData, receivedNestedObject);
	...
	dsd_iot_shadow_disable_discard_old_delta_msgs();
