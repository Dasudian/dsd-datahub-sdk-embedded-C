...
/**
 * @file dsd_iot_tests_unit_connect_helper.c
...
TEST_GROUP_C_SETUP(ConnectTests) {
	...
	rc = dsd_iot_mqtt_disconnect(&iotClient);
	ResetTLSBuffer();
...
TEST_C(ConnectTests, NullClientInit) {
	...
	rc = dsd_iot_mqtt_init(NULL, &initParams);
...
TEST_C(ConnectTests, NullClientConnect) {
	...
	rc = dsd_iot_mqtt_connect(NULL, &connectParams);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
...
TEST_C(ConnectTests, NullHost) {
	...
	InitMQTTParamsSetup(&initParams, NULL, DSD_IOT_MQTT_PORT, false, NULL);
...
TEST_C(ConnectTests, NullPort) {
	...
	InitMQTTParamsSetup(&initParams, DSD_IOT_MQTT_HOST, 0, false, NULL);
	rc = dsd_iot_mqtt_init(&iotClient, &initParams);
	CHECK_C(SUCCESS != rc);
...
TEST_C(ConnectTests, NullClientID) {
	...
	ConnectMQTTParamsSetup(&connectParams, NULL, (uint16_t) strlen(DSD_IOT_MQTT_CLIENT_ID));
	rc = dsd_iot_mqtt_connect(&iotClient, &connectParams);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);

	/* If client id is passed but 0 length was passed, return error */
	ConnectMQTTParamsSetup(&connectParams, DSD_IOT_MQTT_CLIENT_ID, 0);
	rc = dsd_iot_mqtt_connect(&iotClient, &connectParams);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, rc);
...
TEST_C(ConnectTests, ConnectDisconnectConnect) {
	...
	unitTestIsMqttConnected = dsd_iot_mqtt_is_client_connected(&iotClient);
...
TEST_C(ConnectTests, cleanSessionInitSubscribers) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, subTopic1, (uint16_t) strlen(subTopic1), QOS0, iot_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
	...
	snprintf(CallbackMsgStringclean, 100, NO_MSG_XXXX);
	setTLSRxBufferForSuback(subTopic1, strlen(subTopic1), QOS0, testPubMsgParams);
	rc = dsd_iot_mqtt_subscribe(&iotClient, subTopic2, (uint16_t) strlen(subTopic2), QOS0, iot_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
 ...
 * currently the DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS is set to 5
 */
TEST_C(ConnectTests, PowerCycleWithCleanSessionFalse) {
	...
	for(itr = 0; itr < DSD_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS; itr++) {
		...
		rc = dsd_iot_mqtt_subscribe(&iotClient, subTestTopic, subTestTopicLen, QOS0, iot_subscribe_callback_handler,
									NULL);
		CHECK_EQUAL_C_INT(SUCCESS, rc);
	}
