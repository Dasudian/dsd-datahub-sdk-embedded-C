...
/**
 * @file dsd_iot_tests_unit_shadow_json_builder_helper.c
...
TEST_GROUP_C_SETUP(ShadowJsonBuilderTests) {
	...
	ret_val = dsd_iot_shadow_init(&iotClient, &shadowInitParams);
	...
	ret_val = dsd_iot_shadow_connect(&iotClient, &shadowConnectParams);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
...
TEST_C(ShadowJsonBuilderTests, UpdateTheJSONDocumentBuilder) {
	...
	ret_val = dsd_iot_shadow_init_json_document(updateRequestJson, jsonBufSize);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	ret_val = dsd_iot_shadow_add_reported(updateRequestJson, jsonBufSize, 2, &dataDoubleHandler, &dataFloatHandler);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	ret_val = dsd_iot_finalize_json_document(updateRequestJson, jsonBufSize);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
...
TEST_C(ShadowJsonBuilderTests, PassingNullValue) {
	...
	ret_val = dsd_iot_shadow_init_json_document(NULL, SIZE_OF_UPFATE_BUF);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, ret_val);
	ret_val = dsd_iot_shadow_add_reported(NULL, SIZE_OF_UPFATE_BUF, 2, &dataDoubleHandler, &dataFloatHandler);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, ret_val);
	ret_val = dsd_iot_shadow_add_desired(NULL, SIZE_OF_UPFATE_BUF, 2, &dataDoubleHandler, &dataFloatHandler);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, ret_val);
	ret_val = dsd_iot_finalize_json_document(NULL, SIZE_OF_UPFATE_BUF);
	CHECK_EQUAL_C_INT(NULL_VALUE_ERROR, ret_val);
}
...
TEST_C(ShadowJsonBuilderTests, SmallBuffer) {
	...
	ret_val = dsd_iot_shadow_add_desired(updateRequestJson, jsonBufSize, 2, &dataDoubleHandler, &dataFloatHandler);
