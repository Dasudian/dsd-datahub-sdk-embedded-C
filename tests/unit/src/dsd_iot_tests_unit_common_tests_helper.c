...
/**
 * @file dsd_iot_tests_unit_common_tests_helper.h
...
char cbBuffer[DSD_IOT_MQTT_TX_BUF_LEN + 2];

static void iot_tests_unit_common_subscribe_callback_handler(DSD_IoT_Client *pClient, char *topicName, uint16_t topicNameLen, IoT_Publish_Message_Params *params,
				   void *pData) {
...
TEST_GROUP_C_SETUP(CommonTests) {
	...
	IoT_Error_t rc = dsd_iot_mqtt_init(&iotClient, &initParams);
...
TEST_C(CommonTests, NullClientGetState) {
	ClientState cs = dsd_iot_mqtt_get_client_state(NULL);
	CHECK_EQUAL_C_INT(CLIENT_STATE_INVALID, cs);
}

TEST_C(CommonTests, NullClientSetAutoreconnect) {
	IoT_Error_t rc = dsd_iot_mqtt_autoreconnect_set_status(NULL, true);
...
TEST_C(CommonTests, BigMQTTRxMessageIgnore) {
	...
	rc = dsd_iot_mqtt_subscribe(&iotClient, "limitTest/topic1", 16, QOS0, iot_tests_unit_common_subscribe_callback_handler, NULL);
	CHECK_EQUAL_C_INT(SUCCESS, rc);
