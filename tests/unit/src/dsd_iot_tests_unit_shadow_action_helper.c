...
/**
 * @file dsd_iot_tests_unit_shadow_action_helper.c
...
#include <dsd_iot_tests_unit_mock_tls_params.h>
...
#include "../include/dsd_iot_tests_unit_helper_functions.h"
#include "../include/dsd_iot_tests_unit_shadow_helper.h"

#include "dsd_iot_tests_unit_mock_tls_params.h"


#define SIZE_OF_UPDATE_DOCUMENT 200
#define TEST_JSON_RESPONSE_FULL_DOCUMENT "{\"state\":{\"reported\":{\"sensor1\":98}}, \"clientToken\":\"" DSD_IOT_MQTT_CLIENT_ID "-0\"}"
#define TEST_JSON_RESPONSE_DELETE_DOCUMENT "{\"version\":2,\"timestamp\":1443473857,\"clientToken\":\"" DSD_IOT_MQTT_CLIENT_ID "-0\"}"
#define TEST_JSON_RESPONSE_UPDATE_DOCUMENT "{\"state\":{\"reported\":{\"doubleData\":4.090800,\"floatData\":3.445000}}, \"clientToken\":\"" DSD_IOT_MQTT_CLIENT_ID "-0\"}"
#define TEST_JSON_SIZE 120
static DSD_IoT_Client client;
...
static void topicNameFromThingAndAction(char *pTopic, const char *pThingName, ShadowActions_t action) {
	...
	snprintf(pTopic, 100, "$dsd/things/%s/shadow/%s", pThingName, actionBuf);
}
...
TEST_GROUP_C_SETUP(ShadowActionTests) {
	...
	shadowInitParams.pHost = DSD_IOT_MQTT_HOST;
	shadowInitParams.port = DSD_IOT_MQTT_PORT;
	shadowInitParams.pClientCRT = DSD_IOT_CERTIFICATE_FILENAME;
	shadowInitParams.pRootCA = DSD_IOT_ROOT_CA_FILENAME;
	shadowInitParams.pClientKey = DSD_IOT_PRIVATE_KEY_FILENAME;
	shadowInitParams.disconnectHandler = NULL;
	shadowInitParams.enableAutoReconnect = false;
	ret_val = dsd_iot_shadow_init(&client, &shadowInitParams);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);

	shadowConnectParams.pMyThingName = DSD_IOT_MY_THING_NAME;
	shadowConnectParams.pMqttClientId = DSD_IOT_MQTT_CLIENT_ID;
	shadowConnectParams.mqttClientIdLen = (uint16_t) strlen(DSD_IOT_MQTT_CLIENT_ID);
	ConnectMQTTParamsSetup(&connectParams, DSD_IOT_MQTT_CLIENT_ID, (uint16_t) strlen(DSD_IOT_MQTT_CLIENT_ID));
	setTLSRxBufferForConnack(&connectParams, 0, 0);
	ret_val = dsd_iot_shadow_connect(&client, &shadowConnectParams);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	...
	topicNameFromThingAndAction(topic, DSD_IOT_MY_THING_NAME, SHADOW_GET);
...
TEST_GROUP_C_TEARDOWN(ShadowActionTests) {
	 ...
	 */
	IoT_Error_t rc = dsd_iot_shadow_disconnect(&client);
	IOT_UNUSED(rc);
}
...
TEST_C(ShadowActionTests, GetTheFullJSONDocument) {
	...
	dsd_iot_shadow_internal_get_request_json(getRequestJson, TEST_JSON_SIZE);
	ret_val = dsd_iot_shadow_internal_action(DSD_IOT_MY_THING_NAME, SHADOW_GET, getRequestJson, TEST_JSON_SIZE, actionCallback, NULL, 4,
											 false);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	...
	setTLSRxBufferWithMsgOnSubscribedTopic(GET_ACCEPTED_TOPIC, strlen(GET_ACCEPTED_TOPIC), QOS0, params,
										   params.payload);
	ret_val = dsd_iot_shadow_yield(&client, 200);
...
TEST_C(ShadowActionTests, DeleteTheJSONDocument) {
	...
	dsd_iot_shadow_internal_delete_request_json(deleteRequestJson, TEST_JSON_SIZE);
	ret_val = dsd_iot_shadow_internal_action(DSD_IOT_MY_THING_NAME, SHADOW_DELETE, deleteRequestJson, TEST_JSON_SIZE, actionCallback,
											 NULL, 4, false);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
...
TEST_C(ShadowActionTests, UpdateTheJSONDocument) {
	...
	ret_val = dsd_iot_shadow_init_json_document(updateRequestJson, SIZE_OF_UPDATE_DOCUMENT);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	ret_val = dsd_iot_shadow_add_reported(updateRequestJson, SIZE_OF_UPDATE_DOCUMENT, 2, &dataDoubleHandler,
										  &dataFloatHandler);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	ret_val = dsd_iot_shadow_add_desired(updateRequestJson, SIZE_OF_UPDATE_DOCUMENT, 1, &dataBoolHandler);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	ret_val = dsd_iot_finalize_json_document(updateRequestJson, SIZE_OF_UPDATE_DOCUMENT);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	...
	snprintf(expectedUpdateRequestJson, SIZE_OF_UPDATE_DOCUMENT,
			...
			DSD_IOT_MQTT_CLIENT_ID);
	CHECK_EQUAL_C_STRING(expectedUpdateRequestJson, updateRequestJson);

	ret_val = dsd_iot_shadow_internal_action(DSD_IOT_MY_THING_NAME, SHADOW_UPDATE, updateRequestJson, SIZE_OF_UPDATE_DOCUMENT, actionCallback,
											 NULL, 4, false);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
...
#define TEST_JSON_RESPONSE_FULL_DOCUMENT_WITH_VERSION(num) "{\"state\":{\"reported\":{\"sensor1\":98}}, \"clientToken\":\"" DSD_IOT_MQTT_CLIENT_ID "-0\",\"version\":" #num "}"
...
TEST_C(ShadowActionTests, GetVersionFromAckStatus) {
	...
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	CHECK_C(1u == dsd_iot_shadow_get_last_received_version());
	...
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
	CHECK_C(132387u == dsd_iot_shadow_get_last_received_version());
...
TEST_C(ShadowActionTests, ACKWaitingMoreThanAllowed) {
	...
	// 1st
	dsd_iot_shadow_internal_get_request_json(getRequestJson, TEST_JSON_SIZE);
	ret_val = dsd_iot_shadow_internal_action(DSD_IOT_MY_THING_NAME, SHADOW_GET, getRequestJson, TEST_JSON_SIZE, actionCallback, NULL,
...
TEST_C(ShadowActionTests, InboundDataTooBigForBuffer) {
	...
	char getRequestJson[TEST_JSON_SIZE];
	IoT_Publish_Message_Params params;
	char expectedCallbackString[DSD_IOT_MQTT_RX_BUF_LEN + 2];
	...
	for(i = 0; i < DSD_IOT_MQTT_RX_BUF_LEN; i++) {
...
TEST_C(ShadowActionTests, GetAndDeleteRequest) {
	...
	ret_val = dsd_iot_shadow_internal_get_request_json(NULL, TEST_JSON_SIZE);
	CHECK_EQUAL_C_INT(FAILURE, ret_val);
	
	ret_val = dsd_iot_shadow_internal_get_request_json(getRequestJson, 1);
	CHECK_EQUAL_C_INT(FAILURE, ret_val);
	
		ret_val = dsd_iot_shadow_internal_delete_request_json(NULL, TEST_JSON_SIZE);
	CHECK_EQUAL_C_INT(FAILURE, ret_val);
	
	ret_val = dsd_iot_shadow_internal_delete_request_json(getRequestJson, 1);
	CHECK_EQUAL_C_INT(FAILURE, ret_val);
...
TEST_C(ShadowActionTests, NoCallbackForShadowAction) {
	...
	dsd_iot_shadow_internal_get_request_json(getRequestJson, TEST_JSON_SIZE);
	ret_val = dsd_iot_shadow_internal_action(DSD_IOT_MY_THING_NAME, SHADOW_GET, getRequestJson, TEST_JSON_SIZE, NULL, NULL, 4, false);
	CHECK_EQUAL_C_INT(SUCCESS, ret_val);
